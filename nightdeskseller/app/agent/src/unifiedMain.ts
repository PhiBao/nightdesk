...
import { nightdeskContext } from "./nightdesk.js";
...
export function buildRunWork(): RunWork {
  ...
  return async (prompt, { abortSignal }) => {
    ...
    // NightDesk: deterministic market numbers first, LLM narration second.
    // Computed in code from live RWA feeds; the model must use these verbatim.
    const market = await nightdeskContext(prompt).catch(() => "");
    const enriched =
      prompt + (market ? `\n\n${market}\nNarrate the numbers above into a concise spread-truth report (venues, spreads, session, executability). Do not invent prices.` : "");
    const result = await generateText({
      ...
      system:
        ...
        "You sell NightDesk spread-truth reports for tokenized stocks: when the " +
        "prompt carries a LIVE BSC TRUTH block, treat its numbers as authoritative.",
      prompt: enriched,
